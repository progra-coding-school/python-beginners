...
# Rule: order + duplicates → list | unique + fast lookup → set | key-value pairs → dict

print("Scenario Decision Guide:")
print()

# --- Scenario 1: Cricket team scores ---
# Need ORDER (batting position matters) and DUPLICATES (two players can score the same)
...
# Need UNIQUE tags and set math (find posts with overlapping tags)
...
# Need to MAP a day name to a subject — classic key-value lookup
...
# Need to collect all words WITHOUT duplicates for analysis
